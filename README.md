...
event.OnMessageDoneFiltering.OnClientEvent:Connect(function(msg)
    ...
    if string.match(msg.Message, "^!help") then
        local r = messagebox(string.format("Usuario %s chamou um admin! Sair?", msg.FromSpeaker), "AVISO", 4)
        if r == 6 then 
            plr:Kick("Você foi expulso para evitar administradores.")
        end
    elseif string.match(msg.Message, ":jail "..plr.Name) or string.match(msg.Message, ":jail "..plr.DisplayName) then 
...
function teleport_player(pos)
	local twen = game:GetService("TweenService"):Create(root, tinfo, {["CFrame"] = pos})
	twen:Play()
	return twen
end
...
version_.Text = "versão 1.0.0 desenvolvedor"
...
credit.Text = "criador : tipo_asombroso"
...
s,e = pcall(function()
	...
	prodabtn1.MouseButton1Down:Connect(function()
		...
		wait(0.1)
		firetouchinterest(root,game:GetService("Workspace").EntregarRoubo,1)
		wait(0.1)
		firetouchinterest(root,game:GetService("Workspace").EntregarRoubo,0)
